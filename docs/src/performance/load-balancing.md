...
graph TD
    ...
    S -->|round_robin| RR[Advance global counter → pick backend[counter % n]]
    S -->|least_conn| LC[Scan pool → pick backend with lowest active_conns]
    S -->|random| RD[Pick uniformly random healthy backend]
    S -->|ip_hash| IH[FNV-1a hash of client IP → pick backend[hash % n]]
    RR --> F[find_available_from: skip unhealthy / at cap]
    IH --> F
