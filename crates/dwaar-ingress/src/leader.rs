...
fn lease_is_expired(lease: &Lease, duration_secs: i32) -> bool {
    ...
    let MicroTime(ts) = renew_time;
    // Safe cast: duration_secs > 0 after the guard above.
    let duration = duration_secs as u64;
    // `duration_since` returns elapsed time from `ts` to now; clamp to zero so
    // a clock skew in the future doesn't produce a nonsensical negative value.
    let age_secs = k8s_openapi::jiff::Timestamp::now()
        .duration_since(*ts)
        .as_secs()
        .max(0) as u64;
...
///
/// jiff's `Timestamp::Display` with precision 6 emits ISO 8601 / RFC 3339 at
/// microsecond resolution, matching what `MicroTime` expects on the wire.
fn utc_now() -> String {
    format!("{:.6}", k8s_openapi::jiff::Timestamp::now())
}

/// Parse an RFC 3339 timestamp string into a `jiff::Timestamp`.
///
/// Panics only if the timestamp we just generated via `utc_now()` is not
/// parseable — which would indicate a bug in `jiff` itself. Using `expect`
/// rather than `unwrap` so the failure site is identifiable.
fn parse_micro_time(ts: &str) -> k8s_openapi::jiff::Timestamp {
...
mod tests {
    ...
    fn make_lease_with_renew_time(seconds_ago: i64) -> Lease {
        ...
        let renew_ts = Timestamp::now() - SignedDuration::from_secs(seconds_ago);
        Lease {
            metadata: ObjectMeta {
    ...
    fn utc_now_produces_valid_rfc3339() {
        let ts = utc_now();
        let parsed = ts.parse::<k8s_openapi::jiff::Timestamp>();
